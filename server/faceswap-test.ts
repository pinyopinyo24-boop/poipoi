...
async function runFaceSwapTest() {
  console.log("========== 顔入れ替え機能テスト開始 ==========\n");
  ...
  try {
    ...
    if (resultLow.success) {
      ...
      console.log(`   検出ポイント: ソース=${resultLow.details?.sourcePoints}, ターゲット=${resultLow.details?.targetPoints}`);

      // 結果を保存
      const outputPath = path.join(testImagesDir, "result-low.jpg");
    ...
    if (resultMedium.success) {
      ...
      console.log(`   検出ポイント: ソース=${resultMedium.details?.sourcePoints}, ターゲット=${resultMedium.details?.targetPoints}`);

      // 結果を保存
      const outputPath = path.join(testImagesDir, "result-medium.jpg");
    ...
    if (resultHigh.success) {
      ...
      console.log(`   検出ポイント: ソース=${resultHigh.details?.sourcePoints}, ターゲット=${resultHigh.details?.targetPoints}`);
      ...
      // 結果を保存
      const outputPath = path.join(testImagesDir, "result-high.jpg");
    ...
    // レポートをファイルに保存
    const reportPath = path.join(testImagesDir, "performance-report.txt");
...
// テストを実行
runFaceSwapTest().catch(console.error);
