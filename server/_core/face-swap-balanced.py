#!/usr/bin/env python3
"""
Face Swap with Balanced Blending
Stronger blending for visible results while maintaining realism
"""
...
def create_refined_mask(landmarks, shape, blur_strength=201):
...
def swap_face_balanced(source_face_rect, source_landmarks, frame, target_face, target_landmarks):
    """Swap face with balanced blending"""
    try:
        ...
        # Create refined mask
        src_mask = create_refined_mask(target_lm_local, (target_h, target_w), blur_strength=201)
        
        # Get target region
        target_region = frame[ty1:ty2, tx1:tx2].copy()
        
        # Blend with BALANCED strength (0.55)
        mask_float = src_mask.astype(np.float32) / 255.0
        mask_float = mask_float * 0.55  # Balanced blending
        ...
        if mask_threshold.sum() > 100:
            ...
            if src_color_mean.sum() > 0:
                ...
                # Subtle correction (0.6 blend)
                color_correction = color_correction * 0.6 + np.ones(3) * 0.4
...
def process_video_balanced(source_image_path, video_path, output_video_path, fps=24):
    """Process entire video with balanced blending"""
    try:
        # Initialize
        ...
        temp_dir = f"/tmp/faceswap_balanced_{os.getpid()}"
        ...
        print(f"Processing video with balanced blending...", file=sys.stderr)
        ...
        while True:
            ...
            if len(frame_faces) > 0 and frame_faces[0].landmark_2d_106 is not None:
                output_frame = swap_face_balanced(
                    source_face_rect,
...
if __name__ == "__main__":
    ...
    result = process_video_balanced(sys.argv[1], sys.argv[2], sys.argv[3])
