#!/usr/bin/env python3
"""
Face Swap with Scaled Source Face
Makes source face larger and more prominent
"""
...
def scale_landmarks(landmarks, scale_factor, center):
    """Scale landmarks around a center point"""
    scaled = (landmarks - center) * scale_factor + center
    return scaled

def swap_face_scaled(source_face_rect, source_landmarks, frame, target_face, target_landmarks):
    """Swap face with scaled source face"""
    try:
        ...
        # Scale source face to match target size
        ...
        # Calculate scale factor (make source face larger)
        if src_h > 0 and src_w > 0:
            scale_h = target_h_bbox / src_h * 1.2  # 1.2x larger
            scale_w = target_w_bbox / src_w * 1.2
            scale_factor = max(scale_h, scale_w)
        else:
            scale_factor = 1.0
        
        # Scale source landmarks
        scaled_src_landmarks = scale_landmarks(source_landmarks, scale_factor, src_center)
        
        # Resize source face to accommodate scaling
        ...
        # Adjust scaled landmarks for resized image
        ...
        # Create mask from source face contour
        src_mask = np.zeros((target_h, target_w), dtype=np.uint8)
        
        # Transform source contour to target space
        contour_points = scaled_src_landmarks[:33]
        transformed_contour = cv2.perspectiveTransform(
            np.array([contour_points], dtype=np.float32),
            cv2.getPerspectiveTransform(
                np.float32([[0, 0], [target_w, 0], [0, target_h]]),
                np.float32([[0, 0], [target_w, 0], [0, target_h]])
            )
        )[0]
        
        # Use target contour instead
        target_contour = target_lm_local[:33]
        target_contour = np.clip(target_contour, 0, [target_w-1, target_h-1])
        cv2.fillPoly(src_mask, [np.int32(target_contour)], 255)
        
        # Apply strong Gaussian blur for smooth blending
        src_mask = cv2.GaussianBlur(src_mask, (151, 151), 0)
        ...
        src_color_mean = warped_src[src_mask > 127].mean(axis=0)
        tgt_color_mean = target_region[src_mask > 127].mean(axis=0)
...
def process_video_scaled(source_image_path, video_path, output_video_path, fps=24):
    """Process entire video with scaled source face"""
    try:
        # Initialize
        ...
        temp_dir = f"/tmp/faceswap_scaled_{os.getpid()}"
        ...
        print(f"Processing video with scaled source face...", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    result = process_video_scaled(sys.argv[1], sys.argv[2], sys.argv[3])
