#!/usr/bin/env python3
"""
Face Swap with Soft Blending
Reduces blending strength for more realistic appearance
"""
...
def swap_face_soft(source_face_rect, source_landmarks, frame, target_face, target_landmarks):
    """Swap face with soft blending"""
    try:
        ...
        if src_h > 0 and src_w > 0:
            scale_h = target_h_bbox / src_h * 1.1  # Slightly larger
        ...
        # Create mask with SOFT blending
        ...
        # Apply VERY strong Gaussian blur for soft blending (201x201)
        src_mask = cv2.GaussianBlur(src_mask, (201, 201), 0)
        
        # Get target region
        target_region = frame[ty1:ty2, tx1:tx2].copy()
        
        # Blend with REDUCED strength (0.6 instead of 1.0)
        mask_float = src_mask.astype(np.float32) / 255.0
        mask_float = mask_float * 0.6  # Reduce blending strength
        ...
        if mask_threshold.sum() > 100:
            ...
            if src_color_mean.sum() > 0:
                ...
                # Apply subtle correction (0.7 blend)
                color_correction = color_correction * 0.7 + np.ones(3) * 0.3
...
def process_video_soft(source_image_path, video_path, output_video_path, fps=24):
    """Process entire video with soft blending"""
    try:
        # Initialize
        ...
        temp_dir = f"/tmp/faceswap_soft_{os.getpid()}"
        ...
        print(f"Processing video with soft blending...", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    result = process_video_soft(sys.argv[1], sys.argv[2], sys.argv[3])
