...
export async function extractVideoFrames(
  ...
  try {
    ...
    const fps = Math.max(0.1, 1 / (frameInterval / 1000));
    
    try {
      // FFmpeg でフレーム抽出（より安定した設定）
      ...
      const output = execSync(cmd, { stdio: "pipe", encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 });
...
export async function batchFaceSwap(
  ...
  // ソース画像の顔検出を事前に実施
  console.log("[VideoFaceSwap] ソース画像の顔検出中...");
  const sourceBuffer = base64ToBuffer(sourceImageBase64);
  const sourceLandmarks = await detectFaceLandmarks(sourceBuffer);
  
  if (!sourceLandmarks) {
    console.error("[VideoFaceSwap] ソース画像から顔が検出されませんでした");
    return [];
  }

  console.log("[VideoFaceSwap] ソース画像の顔検出成功");
  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    try {
      ...
      if (result.success && result.resultImage) {
        ...
        // フォールバック: 顔検出失敗時は元フレームを使用
        if (result.error?.includes("顔が検出されませんでした")) {
          console.log(`[VideoFaceSwap] フレーム ${i} をスキップ（顔検出失敗）`);
        }
      }
    } catch (error) {
      failCount++;
...
export async function reconstructVideo(
  ...
  try {
    ...
    execSync(cmd, { stdio: "pipe", encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 });
